...
**Deploy executed (2026-08-17):** `git pull` + rebuild + `up -d` on prod; `alembic upgrade head` ran automatically and correctly (verified via `alembic_version` directly, not log output — the container's stdout showed no alembic lines at all despite it genuinely running, worth knowing if this comes up again) — `rag_documents.embedding` confirmed resized to `vector(512)`, all GraphRAG tables present and empty. Structural tables `pg_dump`d (1.2GB, 152MB gzipped) and imported into prod: `jurisdictions` 33, `standards_sources` 32, `standards_frameworks` 684, `standards_items` 561,163, `standards_associations` 669,858 — exact match to local source counts, zero errors.

Backfill against prod hit a real problem on the first two attempts: `docker compose exec`'d processes share the main container's cgroup with the 4 gunicorn workers, and `backfill_standards_embeddings.py --all` loading all ~561k `StandardsItem` rows via one `.scalars().all()` call is a genuinely large one-time allocation — OOM-killed outright at the original `mem_limit: 1g`, then still measured (via cgroup `memory.stat`, `anon` specifically — not reclaimable page cache) sitting right at the ceiling at `3g` before the batch loop was even fully underway. Raised to `6g` (host has 26GB+ free; not a meaningful tradeoff) — plateaued stably around 3.2GB actual usage once the initial load completed, comfortable margin. The more correct fix — paginating that query instead of loading the whole table at once — wasn't done; this was the pragmatic unblock under time pressure, worth revisiting if `mem_limit` ever needs raising again for the same reason.

Running now: **60-90+ items/sec sustained** (faster than the local Ollama run — Voyage's API throughput at batch=100 is higher), zero failures, ETA dropping toward ~100min for the full 548k remaining items (13,100 were already indexed from an earlier partial run before the `mem_limit` bump interrupted it — resumed cleanly, no reprocessing, confirming the resumability design works in practice, not just in theory).
