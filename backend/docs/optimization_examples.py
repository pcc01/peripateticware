...
1. âœ— Looping to load related objects
   for item in items:
       related = db.query(Related).filter(...).first()

2. âœ— Loading all columns when only few needed
   query.all()  # Gets all columns
   
3. âœ— Not using indexes
   CREATE INDEX idx_name ON table(column)
   
4. âœ— Using functions in WHERE clause
   WHERE LOWER(name) = 'value'  # Doesn't use index!
   
5. âœ— N+1 implicit relationships
   for activity in activities:
...
1. âœ“ Use eager loading (selectinload/joinedload)
2. âœ“ Add indexes for filtered columns
3. âœ“ Use specific queries (not full load)
4. âœ“ Test performance after changes
5. âœ“ Monitor slow query logs
6. âœ“ Use appropriate join types
7. âœ“ Cache frequently accessed data
8. âœ“ Profile code to find bottlenecks
"""
