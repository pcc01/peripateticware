...
async def seed_privacy_jurisdictions(
    ...
    try:
        ...
        # 3. Filter to IDs that actually have configs/rules
        verified_ids = await _verify_jurisdictions(db, candidate_ids)

        # 4. Write to organizations.privacy_jurisdiction_ids
        await db.execute(
            text("""
        ...
        logger.error(f"[privacy_seeder] Error seeding org {org_id}: {exc}", exc_info=True)
        return []
