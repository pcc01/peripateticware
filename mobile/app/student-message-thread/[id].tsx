// app/student-message-thread/[id].tsx — full thread for one conversation,
// STUDENT side. Reached from app/student-messages.tsx. Mirrors
// app/teacher-message-thread/[id].tsx structure exactly, including the
// FlatList `style={{flex:1}}` keyboard-covers-input fix and the
// KeyboardAvoidingView `behavior` pattern — see that file's comments.
...
import { fetchStudentThread, replyInStudentThread, StudentThreadMessage } from '@/src/api/studentMessages';

export default function StudentMessageThreadScreen() {
  const { theme } = useTheme();
  const { t } = useTranslation();
  const { id, name } = useLocalSearchParams<{ id: string; name?: string }>();
  const [messages, setMessages] = useState<StudentThreadMessage[]>([]);
  ...
  const load = useCallback(() => {
    if (!id) return;
    fetchStudentThread(id).then(setMessages).catch(() => setError(true)).finally(() => setLoading(false));
  }, [id]);
  ...
  const sendReply = async () => {
    ...
    setSending(true);
    try {
      await replyInStudentThread(id, reply.trim());
      setReply('');
      load();
    } catch {
      Alert.alert(t('common.error', 'Something went wrong'), t('studentMessages.sendError', 'Could not send this message.'));
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <SafeAreaView testID="student-message-thread-screen" style={[styles.root, { backgroundColor: theme.bg }]} edges={['top']}>
      <View style={[styles.header, { borderBottomColor: theme.border }]}>
        <TouchableOpacity testID="student-message-thread-back-btn" onPress={() => router.back()} hitSlop={12} style={styles.backTouchTarget} accessibilityRole="button" accessibilityLabel={t('common.back', 'Back')}>
          <Text style={[styles.backArrow, { color: theme.accent }]}>{'‹'}</Text>
        </TouchableOpacity>
        <Text style={[styles.title, { fontFamily: theme.fontHead, color: theme.text }]} numberOfLines={1}>{name || t('studentMessages.title', 'Messages')}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }} keyboardVerticalOffset={90}>
        {loading ? (
          ...
          <FlatList
            testID="student-message-thread-list"
            // Keep the flex:1 fix from app/teacher-message-thread/[id].tsx:
        ...
        <View style={[styles.replyRow, { borderTopColor: theme.border, backgroundColor: theme.surface }]}>
          <TextInput
            testID="student-message-thread-reply-input"
            value={reply}
            onChangeText={setReply}
            placeholder={t('studentMessages.replyPlaceholder', 'Write a reply…')}
          ...
          />
          <TouchableOpacity
            testID="student-message-thread-send"
            onPress={sendReply}
            disabled={!reply.trim() || sending}
            style={[styles.sendBtn, { backgroundColor: theme.accent, opacity: !reply.trim() || sending ? 0.5 : 1 }]}
          >
            {sending ? <ActivityIndicator color="#fff" size="small" /> : <Text style={{ color: '#fff', fontWeight: '700' }}>{t('studentMessages.send', 'Send')}</Text>}
          </TouchableOpacity>
        </View>
