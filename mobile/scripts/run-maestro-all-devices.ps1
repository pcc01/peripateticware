...
foreach ($row in $matrix) {
    ...
    try {
        ...
        try {
            # No LOCATION_PERMISSION env var — the affected flows grant it
            # via a platform-gated launchApp block instead (see
            # maestro/flows/capture/12.1-note-capture.yaml's header).
