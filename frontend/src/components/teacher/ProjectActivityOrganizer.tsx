  ...
  if (orderedActivities.length === 0) {
    return (
      <div className={clsx(
        ...
        <p className="text-gray-600 dark:text-gray-400 font-medium">{t("landing:no_activities", "No Activities")}</p>
        <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">{t("landing:add_activities_to_organize_them_in_this_", "Add activities to organize them in this project")}
  ...
  return (
    <div className={clsx(
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-gray-900 dark:text-white">{t("landing:projectactivityorganizer.project_activities", "\uD83D\uDCCB Project Activities (")}
        ...
        <div className="flex gap-2">
            ...
            variant="secondary"
            size="sm"
            onClick={handleUndo}>{t("landing:undo", "Undo")}
            ...
            disabled={loading}
            isLoading={loading}>{t("landing:save_order", "Save Order")}
      ...
      {/* Help Text */}
      {hasChanges &&
      <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 text-xs">{t("landing:drag_activities_to_reorder_them_click_sa", "Drag activities to reorder them. Click \"Save Order\" to apply changes.")}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  <span className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded">{t("landing:projectactivityorganizer.grade", "Grade")}
                  {activity.grade_level}
                  </span>
                  <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-0.5 rounded">
                    {activity.estimated_duration_minutes}{t("landing:min", "min")}
                </span>
                </div>
              </div>
              ...
              title={t("landing:remove_activity_from_project", "Remove activity from project")}>
      ...
      {/* Change Summary */}
      {hasChanges &&
      <div className="p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 text-xs">{t("landing:you_have_unsaved_changes_to_the_activity", "You have unsaved changes to the activity order. Click \"Save Order\" to apply them.")}

      </div>
      }

      {/* Info */}
      <p className="text-xs text-gray-500 dark:text-gray-400 text-center pt-2 border-t border-gray-200 dark:border-gray-700">{t("landing:showing", "Showing")}
        {orderedActivities.length}{t("landing:activit", "activit")}{orderedActivities.length !== 1 ? 'ies' : 'y'}
      </p>
    </div>);
