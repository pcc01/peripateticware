...
export const CurriculumMapper: React.FC<CurriculumMapperProps> = ({
  ...
  return (
    <div className={clsx(
      ...
      <h3 className="font-semibold text-gray-900 dark:text-white">{t("landing:map_curriculum_standards", "\uD83D\uDCDA Map Curriculum Standards")}
      ...
      <div className="space-y-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
        {/* Search Query */}
        <div>
          <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t("landing:search_curriculum", "Search Curriculum")}
          ...
          <input
            ...
            placeholder={t("landing:search_by_name_or_topic", "Search by name or topic...")}
            className={clsx(
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t("landing:subject", "Subject")}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">{t("landing:curriculummapper.grade", "Grade")}
            ...
            <select
              ...
              {GRADES.map((grade) =>
              <option key={grade} value={grade}>{t("landing:curriculummapper.grade", "Grade")}
                {grade}
                </option>
              )}
            </select>
        ...
        <Button
          ...
          disabled={loading}
          isLoading={loading}
          className="w-full">{t("landing:search_curriculum", "Search Curriculum")}
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
          <p className="text-xs font-medium text-gray-700 dark:text-gray-300 px-2">{t("landing:found", "Found")}
          {totalResults}{t("landing:result", "result")}{totalResults !== 1 ? 's' : ''}
          </p>
          {results.map((unit) =>
        <div
          key={unit.id}
          ...
          onClick={() => toggleUnitSelection(unit)}>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 line-clamp-1">
                ...
                <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                    ...
                    {unit.grade_level > 0 &&
                <span className="text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded">{t("landing:curriculummapper.grade", "Grade")}
                  {unit.grade_level}
                      </span>
                }
                    {unit.bloom_level > 0 &&
      ...
      <div className="text-center py-6 text-gray-500 dark:text-gray-400">
          <p className="text-sm">{t("landing:no_curriculum_units_found", "No curriculum units found")}</p>
        </div>
      }
      ...
      <div className="space-y-2 pt-4 border-t border-gray-200 dark:border-gray-700">
          <p className="text-xs font-medium text-gray-700 dark:text-gray-300">{t("landing:selected_units", "Selected Units (")}
