...
const ProgressDashboard: React.FC<ProgressDashboardProps> = ({ studentId }) => {
  ...
  if (loading) return <div className="p-4 text-center">{t("landing:loading_progress", "Loading progress...")}</div>;
  if (error) return <div className="p-4 text-red-600">{error}</div>;
  if (!dashboard) return <div className="p-4 text-gray-600">{t("landing:no_progress_data_available", "No progress data available")}</div>;

  return (
    <div className="space-y-6 p-4">
      <h3 className="font-semibold text-lg">{t("landing:learning_progress", "Learning Progress")}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white border rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t("landing:progressdashboard.activities", "Activities")}</p>
        ...
        <div className="bg-white border rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm">{t("landing:progressdashboard.captures", "Captures")}</p>
        ...
        <div className="bg-white border rounded-lg p-4">
          <div>
            <p className="text-gray-600 text-sm">{t("landing:streak", "Streak")}</p>
            <p className="text-2xl font-bold">{dashboard.engagement_streak_days}{t("landing:days", "days")}</p>
          </div>
        </div>
        <div className="bg-white border rounded-lg p-4">
          <div>
            <p className="text-gray-600 text-sm">{t("landing:competencies", "Competencies")}</p>
      ...
      <div className="bg-white border rounded-lg p-4">
        <h4 className="font-semibold mb-4 flex items-center gap-2">
          <TrendingUp className="w-5 h-5" />{t("landing:competency_progress", "Competency Progress")}
      ...
      {/* Engagement Trend */}
      <div className="bg-white border rounded-lg p-4">
        <h4 className="font-semibold mb-4">{t("landing:weekly_engagement", "Weekly Engagement")}</h4>
