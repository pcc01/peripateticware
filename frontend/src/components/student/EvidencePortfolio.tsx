...
const EvidencePortfolio: React.FC<EvidencePortfolioProps> = ({ studentId }) => {
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div key={collection.id} className="border rounded-lg p-6 bg-white">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-gray-600">{t("landing:evidenceportfolio.items", "Items:")}</span>
              <p className="font-semibold">{collection.captures.length + collection.entries.length}</p>
            </div>
            <div>
              <span className="text-gray-600">{t("landing:evidenceportfolio.status", "Status:")}</span>
  ...
  if (loading) return <div className="p-4 text-center">{t("landing:loading_portfolio", "Loading portfolio...")}</div>;
  if (error) return <div className="p-4 text-red-600">{error}</div>;

  return (
    <div className="space-y-4 p-4 border rounded-lg">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-lg">{t("landing:evidenceportfolio.evidence_portfolio", "Evidence Portfolio")}</h3>
